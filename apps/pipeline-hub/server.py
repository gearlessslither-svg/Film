...
from analyze_aigc_project import analyze_stages, count_files_by_category, list_files, shot_list_stats  # noqa: E402
...
def project_detail(slug: str, include_report_text: bool = True) -> dict[str, object]:
...
def list_projects() -> list[dict[str, object]]:
    ...
    for path in sorted(PROJECTS_ROOT.iterdir()):
        ...
        if not (path / "project.yaml").exists():
            continue
        detail = project_detail(path.name, include_report_text=False)
